    ...
    temporaryMap.map((line) => {
        ...
        for (let x = 0; x < line.length; x++) {
            ...
            if (WALLS.includes(element)) {
                ...
                $("<div>&nbsp;</div>").addClass("square ground")
                    .appendTo(container);
            } else if (element === "#") {
                $(`<div>${element}</div>`)
                    .addClass("square box")
...
/**
 * This method display amount of previous move avalaible
 */
function displayPreviousMoveCount() {
    if (states.length < 1) {
        $(".back").text("Retour en arrière (0 possibles)");
    } else {
        $(".back").text(`Retour en arrière (${states.length - 1} possibles)`);
    }
}
 ...
 */
function getSquareAt(position, level) {
    // eslint-disable-next-line init-declarations
    let element = "";
    temporaryMap.forEach((line) => {
...
function isWall(position) {
    const square = getSquareAt(position, actualLevel);
...
function allOnTarget() {
    return currentTargets.every(
        (pos) => getSquareAt(pos, actualLevel) === VALIDATED_BOX
    );
}
    ...
    if (update.box) {
        if (update.posBox) {
            temporaryMap[update.posBox.y] = replaceAt(
                ...
                update.posBox.x,
                getSquareAt(update.posBox, actualLevel) === TARGET
                    ? VALIDATED_BOX
                    : BOX
            );
        }
...
function move(direction) {
    ...
    if (allOnTarget() && actualLevel !== 6) {
        setTimeout(() => {
            alert(`Vous avez terminé le niveau, appuyez sur la touche ESPACE pour passer au niveau ${actualLevel + 2}`);
        }, 20);
    }
}
...
function backToPreviousMove() {
    if (states.length !== 0) {
        const penultimateMove = states[states.length - 2]; // (position précédente du joueur, là ou il était avant ${lastMove})
        const lastMove = states[states.length - 1]; // (position actuelle du joueur, on l'utilise pour déplacer la box s'il y en a une)
        if (penultimateMove) {
            ...
            temporaryMap[lastMove.playerPosition.y] = replaceAt(
                ...
                currentTargets.some((target) => lastMove.playerPosition.x === target.x && lastMove.playerPosition.y === target.y) ? TARGET : " "
            );
            ...
            if (lastMove.boxPosition !== undefined) {
                ...
                temporaryMap[lastMove.boxPosition.y] = replaceAt(
                    ...
                    getSquareAt(lastMove.boxPosition, actualLevel) === VALIDATED_BOX ?
                        TARGET :
                        " "
                );
                ...
                temporaryMap[lastMove.playerPosition.y] = replaceAt(
                    temporaryMap[lastMove.playerPosition.y],
                    lastMove.playerPosition.x,
                    getSquareAt(lastMove.playerPosition, actualLevel) === TARGET ?
                        VALIDATED_BOX :
                        BOX
                ); //
            }
            displayPreviousMoveCount();
        } else {
